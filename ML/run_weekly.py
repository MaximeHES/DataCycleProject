...
if __name__ == "__main__":
    ...
    for i, step in enumerate(PIPELINE):
        ...
        if not success:
            print(f"\n{'=' * 55}")
            print(f"  WEEKLY PIPELINE STOPPED")
    ...
    print(f"\n{'=' * 55}")
    print(f"  WEEKLY PIPELINE -- DONE")
    print(f"  Total time : {elapsed}s ({elapsed // 60}m {elapsed % 60}s)")
    print(f"  Outputs    : {OUTPUT_DIR}")
    print(f"  Logs saved : {LOGS_DIR}")
    print(f"{'=' * 55}")

    summary_path = os.path.join(LOGS_DIR, f"weekly_summary_{date_str}.txt")
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write("=" * 55 + "\n")
        f.write("  WEEKLY PIPELINE -- SUMMARY\n")
        f.write("=" * 55 + "\n\n")
